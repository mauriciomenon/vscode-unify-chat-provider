...
function toPersistableConfig(
  config: OpenAICodexAuthConfig | undefined,
): OpenAICodexAuthConfig {
  return {
    method: 'openai-codex',
    label: config?.label,
...
export class OpenAICodexAuthProvider implements AuthProvider {
  ...
  static async resolveForExport(
    auth: OpenAICodexAuthConfig,
    secretStore: SecretStore,
  ): Promise<OpenAICodexAuthConfig> {
  ...
  static async normalizeOnImport(
    ...
    options: {
      ...
      existing?: OpenAICodexAuthConfig;
    },
  ): Promise<OpenAICodexAuthConfig> {
  ...
  static async prepareForDuplicate(
    ...
    const cleared: OpenAICodexAuthConfig = {
  ...
  constructor(
    ...
    private config?: OpenAICodexAuthConfig,
  ) {}
  ...
  getConfig(): OpenAICodexAuthConfig | undefined {
    return this.config;
  }
  ...
  private async resolveTokenData(): Promise<OAuth2TokenData | null> {
    ...
    return OpenAICodexAuthProvider.parseTokenData(raw);
  }
  ...
  async configure(): Promise<AuthConfigureResult> {
    ...
    const authorization = authorizeOpenAICodex();
    ...
    const exchanged = await exchangeOpenAICodexCode({
  ...
  async refresh(): Promise<boolean> {
    ...
    if (refreshed.type === 'failed') {
      authLog.error(
        `${this.context.providerId}:openai-codex`,
        `Refresh failed: ${refreshed.error}`,
      );
